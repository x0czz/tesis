...
\usepackage{etoolbox}         % utilidades para modificar entornos
\usepackage{adjustbox}        % escalado automatico de tablas/figuras
...
\makeatletter
\newcommand*\facsoheadmarkpage{%
  \if@mainmatter
    \thepage
  \else
    \MakeUppercase{\roman{page}}%
  \fi}
\makeatother
\lehead{\facsoheadmarkpage}
\lohead{\facsoheadmarkpage}
\chead{}
\rehead{\itshape\leftmark}
\rohead{\itshape\leftmark}
...
\usepackage{xcolor}
\definecolor{facsolink}{rgb}{0.05,0.2,0.5}
\usepackage{hyperref}
\AtBeginDocument{%
  \hypersetup{
    colorlinks=true,
...
\usepackage{tocloft}
\usepackage{float} % allow H placement for tables
...
\renewcommand{\listoffigures}{%
  \facso@iflist{lof}{figure}{%
    \cleardoublepage
    \phantomsection
    \addchap*{Listado de Figuras}% use KOMA to set heading and marks
    \thispagestyle{scrheadings}% ensure header line is shown
    \markboth{Listado de Figuras}{Listado de Figuras}% keep header label visible
...
\renewcommand{\listoftables}{%
  \facso@iflist{lot}{table}{%
    \cleardoublepage
    \phantomsection
    \addchap*{Listado de Tablas}% use KOMA to set heading and marks
    \thispagestyle{scrheadings}% ensure header line is shown
    \markboth{Listado de Tablas}{Listado de Tablas}% keep header label visible
...
\newcommand{\facsofrontheaderline}[1]{\markboth{#1}{#1}}
\renewcommand{\tableofcontents}{%
  \cleardoublepage
  \phantomsection
  \addchap*{\contentsname}% use KOMA to set heading and marks
  	hispagestyle{scrheadings}% ensure header line is shown
  \markboth{\contentsname}{\contentsname}% keep header label visible
...
% Reduce top space for floats to avoid large gaps
\makeatletter
\setlength{\@fptop}{0pt}
...
% ---------- Limitar ancho de tablas al area de texto ----------
...
\facsofrontheaderline{\contentsname}
\tableofcontents
\cleardoublepage
\facsofrontheaderline{Listado de Figuras}
\listoffigures
\cleardoublepage
\facsofrontheaderline{Listado de Tablas}
\listoftables
\cleardoublepage
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.0606}}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.2118}}@{}}
